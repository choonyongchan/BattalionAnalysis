/**
 * The report-sick flow: a count-only Sankey from the parade state through FormSG to the
 ...
 *
 * **Nothing is reconciled.** No name, 4D or company matching links one stage to the next;
 * each stage is a plain count, and the counts are poured into the next stage in order
 * (`channel_`). So when fewer soldiers report sick on the parade state than on FormSG,
 * every `Reporting sick` flows into `Reported sick` and the surplus submissions come from
 * `Unaccounted`; when more do, the surplus parade-state reports end at `No FormSG
 * submission`. The same rule carries types into outcomes and Status into its buckets: a
 * stage can never carry more than flowed into it, so outcome and bucket counts past that
 * are left out of the diagram and reported in `coverage` instead.
 *
...
/** @type {string} The outcome-stage node for MC. */
const NODE_MC = 'Outcome: MC';

/** @type {string} The outcome-stage node for Status. */
const NODE_STATUS = 'Outcome: Status';

/** @type {string} The outcome-stage node for reports with no outcome to account for them. */
const NODE_NONE = 'Outcome: None recorded';
...
/**
 * Sums the values of links entering a node.
 * @param {Array<{target: string, value: number}>} links Links to sum.
 * @param {string} node The target node name.
 * @returns {number} Total inflow.
 */
function inflow_(links, node) {
...
/**
 * Counts submissions per type stage node, in `REPORT_SICK_TYPES` order.
 * @param {Array<!Object>} submissions Normalised submissions.
 * @returns {Array<{name: string, count: number}>} Type nodes and their counts.
 */
function typeCounts_(submissions) {
  const labels = REPORT_SICK_TYPES.map((type) => type.label).concat(TYPE_NOT_RECORDED);
  const counts = new Map(labels.map((label) => [label, 0]));
  submissions.forEach((submission) => {
    const code = reportSickTypeOf(submission);
    const type = REPORT_SICK_TYPES.find((entry) => entry.name === code);
    const label = type ? type.label : TYPE_NOT_RECORDED;
    counts.set(label, counts.get(label) + 1);
  });
  return labels.map((label) => ({ name: 'Type: ' + label, count: counts.get(label) }));
}

/**
 * Counts Status episodes per restriction bucket, in `STATUS_BUCKETS` order.
 * @param {Array<!Object>} statusEpisodes In-range Status episodes.
 * @returns {Array<{name: string, count: number}>} Bucket nodes and their counts.
 */
function bucketCounts_(statusEpisodes) {
  const counts = new Map();
  statusEpisodes.forEach((episode) => {
    const buckets = new Set(episode.reasons.flatMap((reason) => bucketsFor(reason)));
    buckets.forEach((bucket) => counts.set(bucket, (counts.get(bucket) || 0) + 1));
  });
  return STATUS_BUCKETS.filter((name) => counts.has(name))
    .map((name) => ({ name: 'Status: ' + name, count: counts.get(name) }));
}
...
export function reportSickFlow({ episodes, submissions, from, to }) {
  ...
  const mc = episodesInRange_(episodes, DUTY_CLASS.ATT_C, from, to).length;
  const statusEpisodes = episodesInRange_(episodes, DUTY_CLASS.STATUS, from, to);
  ...
  const types = typeCounts_(subsInRange);
  const typeLinks = channel_([{ name: NODE_REPORTED, count: reportedSick }], types);
  const outcomeLinks = channel_(types, [
    { name: NODE_MC, count: mc },
    { name: NODE_STATUS, count: statusEpisodes.length },
    { name: NODE_NONE, count: Math.max(0, reportedSick - mc - statusEpisodes.length) },
  ]);
  const statusFlow = inflow_(outcomeLinks, NODE_STATUS);
  const bucketLinks = channel_([{ name: NODE_STATUS, count: statusFlow }], bucketCounts_(statusEpisodes));

  const links = [...reportLinks, ...typeLinks, ...outcomeLinks, ...bucketLinks];
  return {
    ...
    coverage: {
      ...
      reportedSick,
      mc,
      status: statusEpisodes.length,
      outcomesNotShown: mc + statusEpisodes.length - inflow_(outcomeLinks, NODE_MC) - statusFlow,
    },
  };
}
